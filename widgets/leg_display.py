...
class LegFunctions:
    ...
    def getDiagPoints(self, start: Tuple[float, float], end: Tuple[float, float], start_width: float, end_width: float, weight: float):
        points = []
        x1, y1 = start
        x2, y2 = end
        ...
        width = np.arange(start_width, end_width + 2*delta_width, delta_width).tolist()
...
class SideLegFunctions(LegFunctions):
    ...
    def getPoints(self):
        ...
        adjusted_thigh_end = [
            current_thigh_end[0] + sign * self.knee_width * (1-math.sin(math.radians(90-self.knee_deg))),
            current_thigh_end[1] + self.knee_width * math.sin(math.radians(self.knee_deg))
        ]
        current_thigh_start = [
            adjusted_thigh_end[0] + sign * math.sin(math.radians(self.knee_deg)) * 0.289,
            adjusted_thigh_end[1] + math.sin(math.radians(90-self.knee_deg)) * 0.289
        ]
        knee_hip_midpoint = [(current_thigh_start[0] + adjusted_thigh_end[0])/2, (current_thigh_start[1] + adjusted_thigh_end[1])/2]
        if (adjusted_thigh_end[0] != current_thigh_start[0] and adjusted_thigh_end[1] != current_thigh_start[1]):
            knee_start = [current_thigh_end[0]-sign*self.knee_width, current_thigh_end[1]]
            slope = (adjusted_thigh_end[1]-current_thigh_start[1])/(adjusted_thigh_end[0]-current_thigh_start[0])
            perp_slope = -1/slope
        ...
        points += self.getDiagPoints(knee_hip_midpoint, adjusted_thigh_end, self.thigh_width, self.knee_width, weight)
        # add points from hip to middle of thigh
        points += self.getDiagPoints(knee_hip_midpoint, current_thigh_start, self.thigh_width, self.hip_width, weight)
